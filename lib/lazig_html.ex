defmodule LazigHTML do
  ...
  #   case LazigHTML.Zigler.from_document_zig(html) |> dbg() do
  #     {:ok, %LazigHTML{resource: resource}} -> %LazigHTML{resource: resource}
  ...
  def from_fragment(html) when is_binary(html) do
    case LazigHTML.Zigler.from_fragment(html) |> dbg() do
      {:ok, %LazigHTML{resource: resource}} -> %LazigHTML{resource: resource}
      {:error, reason} -> {:error, reason}
      result -> result
    end
  ...
  #   '''
  # def to_html(%LazigHTML{} = lazig_html, opts \\ []) when is_list(opts) do
  #   opts = Keyword.validate!(opts, skip_whitespace_nodes: false)

  #   # Convert to LazyHTML struct for ZiglerParser
  #   lazig_html_struct = %LazigHTML{resource: lazig_html.resource}
  #   LazigHTML.Zigler.to_html_zig(lazig_html_struct, opts[:skip_whitespace_nodes])
  # end
  ...
  #     try do
  #       lazy_html_struct = %LazigHTML{resource: lazy_html.resource}
  #       # Use Zigler NIFs for node count and HTML serialization
  #       nodes = LazigHTML.Zigler.nodes(lazy_html_struct)
  ...
  #   defp node_to_doc({%LazigHTML{} = node, number}, opts) do
  #     html_doc =
  #       node
  #       |> LazigHTML.Zigler.to_html_zig()
